...
*/
        ...
        case BtAnd(l, r) => GBtAnd(CDL2GExpression(l), CDL2GExpression(r))
        case BtOr(l, r) => GBtOr(CDL2GExpression(l), CDL2GExpression(r))
        case BtXor(l, r) => GBtXor(CDL2GExpression(l), CDL2GExpression(r))
        case BtLeft(l, r) => GBtLeft(CDL2GExpression(l), CDL2GExpression(r))
        case BtRight(l, r) => GBtRight(CDL2GExpression(l), CDL2GExpression(r))
        ...
        case FunctionCall("is_enabled", List(e)) => GIsEnabledFunc(CDL2GExpression(e))
    ...
    for(node <- allNodes if node.cdlType == InterfaceType) {
            ...
            node match {
                    ...
                    case _ =>
                            ...
                            val n_data = getImpl(id).foldLeft[GExpression](GLongIntLiteral(0))((c, id) =>
                                    c + GConditional(GAliasReference(id+"_effective"), GLongIntLiteral(1), GLongIntLiteral(0)))
    ...
    IF[(CompilationOptions.TRANSFORM_INTERFACE)#v] {
            def transformSingleInterface(t:Any):Any = {
                    ...
                    if ((expr::expr.children).foldLeft(false)((result, child)=>result || (child match {
                            ...
                            expr match {
                                case GEq(GAliasReference(i), GLongIntValue(1)) => GAliasReference(featureID(i) + "_xorGroup")
                                case GEq(GLongIntValue(1), GAliasReference(i)) => GAliasReference(featureID(i) + "_xorGroup")
                                case GEq(GAliasReference(i), GLongIntValue(0)) => GAliasReference(featureID(i) + "_allNegativeGroup")
                                case GEq(GLongIntValue(0), GAliasReference(i)) => GAliasReference(featureID(i) + "_allNegativeGroup")
                                case GLessThan(GAliasReference(i), GLongIntValue(1)) => GAliasReference(featureID(i) + "_allNegativeGroup")
                                case GGreaterThan(GLongIntValue(1), GAliasReference(i)) => GAliasReference(featureID(i) + "_allNegativeGroup")
                                case GGreaterEqThan(GLongIntValue(1), GAliasReference(i)) => GAliasReference(featureID(i) + "_allNegativeGroup") | GAliasReference(featureID(i) + "_xorGroup")
                                case GLessEqThan(GAliasReference(i), GLongIntValue(1)) => GAliasReference(featureID(i) + "_allNegativeGroup") | GAliasReference(featureID(i) + "_xorGroup")
                                case GGreaterThan(GAliasReference(i), GLongIntValue(0)) => GAliasReference(featureID(i) + "_orGroup")
                                case GGreaterEqThan(GAliasReference(i), GLongIntValue(1)) => GAliasReference(featureID(i) + "_orGroup")
                                case GLessThan(GLongIntValue(0), GAliasReference(i)) => GAliasReference(featureID(i) + "_orGroup")
                                case GBoolFunc(GAliasReference(i)) => GAliasReference(featureID(i) + "_orGroup")
                                case GGreaterThan(GAliasReference(i), GLongIntValue(1)) => GAliasReference(featureID(i) + "_twoPositiveGroup")
                                case GLessThan(GLongIntValue(1), GAliasReference(i)) => GAliasReference(featureID(i) + "_twoPositiveGroup")
                                case _ => expr
                            }
                    }
                    else expr
